...
class ReservationController extends Controller
{
    ...
    // Get the table IDs for reservations on the same date
         $res_table_ids = Reservation::orderBy('res_date')->get()->filter(function ($value) use ($reservation) {
         $res_date = Carbon::parse($value->res_date);
            return $res_date->format('Y-m-d') == $reservation->res_date->format('Y-m-d');
            })->pluck('table_id');

    // Fetch tables that are available for the given conditions
